from tkinter import Tk, Canvas, PhotoImage, Button, Label
import math

# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None
is_running = False


# ---------------------------- TIMER RESET ------------------------------- #
def reset():
    global reps, is_running, timer
    if timer is not None:
        window.after_cancel(timer)
        timer = None
        
    timer_label.config(text="Timer", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 50))
    check_label.config(text="", bg=YELLOW)
    canvas.itemconfig(timer_text, text="00:00")
    reps = 0
    is_running = False


# ---------------------------- TIMER MECHANISM ------------------------------- #
def start():
    global reps, is_running
    if is_running:
        return

    is_running = True
    work_sec = WORK_MIN * 60
    short_break_sec = SHORT_BREAK_MIN * 60
    long_break = LONG_BREAK_MIN * 60

    reps += 1
    if reps % 8 == 0:
        timer_label.config(text="Break", fg=RED, bg=YELLOW, font=(FONT_NAME, 50))
        count_down(long_break, final=True)
    elif reps % 2 == 0:
        timer_label.config(text="Break", fg=PINK, bg=YELLOW, font=(FONT_NAME, 50))
        count_down(short_break_sec)
    else:
        timer_label.config(text="Work", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 50))
        count_down(work_sec)


# ---------------------------- COUNTDOWN MECHANISM ------------------------------- #
def count_down(count, final=False):

    check = "✔"

    count_min = math.floor(count / 60)
    if count_min < 10:
        count_min = f"0{count_min}"
    count_sec = count % 60
    if count_sec < 10:
        count_sec = f"0{count_sec}"

    canvas.itemconfig(timer_text, text=f"{count_min}:{count_sec}")
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count - 1, final)
    else:
        if final:
            reset()
        else:
            start()
            if reps % 2 == 0:
                check_label.config(text=f"{int(reps/2)*check}", fg=GREEN, bg=YELLOW)


# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
window.title("Pomodoro")
window.config(padx=100, pady=50, bg=YELLOW)

canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
tomato_img = PhotoImage(file="tomato.png")
canvas.create_image(100, 112, image=tomato_img)
timer_text = canvas.create_text(
    100, 130, text="00:00", fill="white", font=(FONT_NAME, 35, "bold")
)
canvas.grid(row=1, column=1)


start_button = Button(text="Start", command=start, highlightbackground=YELLOW)
start_button.config(justify="right")
start_button.grid(row=2, column=0)

reset_button = Button(text="Reset", command=reset, highlightbackground=YELLOW)
reset_button.config(justify="left")
reset_button.grid(row=2, column=2)

timer_label = Label(text="Timer", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 50))
timer_label.grid(row=0, column=1)

check_label = Label(bg=YELLOW)
check_label.grid(row=3, column=1)

window.mainloop()
